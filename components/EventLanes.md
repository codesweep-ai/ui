...
A mark begins at its position and is as wide as the gap to the next mark anywhere in its timeline, less a one-pixel gutter. A mark with `anchor: "end"` ends at its position instead, drawn to the left of its moment, and while any mark is anchored so the axis keeps a mark size before the first position, as it always keeps one after the last. An opening drawn at the same moment a box begins would otherwise sit inside the box, over its label; anchored at its end it sits just before it, as a reply sits just after. The width is clamped between one pixel and the mark size that `cellWidth` gives. Dense runs therefore read as hairlines, and sparse runs stay readable. The last mark in a timeline draws at the mark size. A mark with an `extent` draws at its true length instead, with no upper clamp.
